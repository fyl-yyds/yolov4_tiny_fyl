...
from data.utils import (DecodeBox, letterbox_image, non_max_suppression,
                         yolo_correct_boxes)
...
class YOLO(object):
    _defaults = {
        "model_path"        : 'logs/Epoch6-Total_Loss5.9088-Val_Loss9.9776.pth',
    ...
    def __init__(self, **kwargs):
        ...
        self.anchors = self._get_anchors()  #[2,3,2]的6个框
        self.generate()
    ...
    def _get_class(self):
        ...
        return class_names  #['face', 'eye', 'mouth', 'left hand', 'right hand']
    ...
    def _get_anchors(self):
        ...
        return np.array(anchors).reshape([-1, 3, 2])  #[2,3,2]6个长宽的框
    ...
    def generate(self):
        ...
        for i in range(2):
            self.yolo_decodes.append(DecodeBox(np.reshape(self.anchors,[-1,2])[self.anchors_mask[i]], len(self.class_names),  (self.model_image_size[1], self.model_image_size[0])))
                                                #anchors[2,3,2]->[6,2]->[3,2];;;5;;;(416,416)
    ...
    #   检测图片
    #---------------------------------------------------#
    def detect_image(self, image,isCut=False):
        #---------------------------------------------------------#
        ...
        #输出框框的数据，原来的的代码就输出了标号框框的图片，这里增加数据框框的数据用于后处理。
        outbox=[]
        ...
        for i, c in enumerate(top_label):
            ...
            outbox.append([c,score,left,top,right,bottom])
